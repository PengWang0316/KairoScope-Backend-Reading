...
const handler = async (event, context, callback) => {
  ...
  const result = await cloudwatch.trackExecTime('MongoDBFindLatency', () => new Promise((resolve, reject) => {
    getDB().collection(readingCollectionName)
      .find(context.user.role * 1 === process.env.ADMINISTRATOR_ROLE * 1 ? {} : { user_id: context.user._id })
      .sort({ date: -1 })
      ...
      .toArray((err, readingResult) => {
        if (err) {
          ...
          resolve({});
        } else if (readingResult.length !== 0) findHexagramImages(readingResult, backResult => resolve(backResult));
